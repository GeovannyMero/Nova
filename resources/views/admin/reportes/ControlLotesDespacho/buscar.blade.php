...
function buscar(){
	...
	alert(idCamion);
	route = "{{ url('/ver/controlLotesDespacho')}}" + '/' + desde + '/' + hasta + '/' + cliente + '/' + idCamion;
	$.ajax({
		type: 'GET',
		url: route,
...
function exportExcel(){
	...
	var hasta = $('#fh').val();
	var cliente = $('#aPlaca').val();
	route = "{{url('/reporteExcel')}}" + '/' + desde + '/' + hasta + '/' + cliente;
	$.ajax({
...
function x(e){
 ...
 window.open('data:application/vnd.ms-excel,' + $('#ex').html());
 e.preventDefault();
}
