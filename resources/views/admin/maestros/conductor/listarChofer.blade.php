		...
		<table id="example-1" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
			...
			@foreach($chofer as $i)
					...
					<tr>
						...
						<td>{{ $i->cedula }}</td>
						<td>{{ $i->nombre}}</td>
						<td>{{ $i->marca . ' ' . $i->placa}}</td>
						<td>{{ $i->cia}}</td>

						<td>
							<a href="{{ url('conductor/edit',$i->eCodReg) }}"><button class="btn btn-secondary"><i class="fa fa-pencil"></i></button></a>
...
function eliminar(id){
	//alert(id);
bootbox.confirm("Est&aacute; seguro que desea eliminar el conductor con c&oacute;digo No "+id+"?",function(result){
	    if(result){
	    route = "{{url('conductor/delete')}}" + "/" + id;
	    	$.ajax({
	    		type: 'GET',
	    		url: route ,
	    		 success: function() {
