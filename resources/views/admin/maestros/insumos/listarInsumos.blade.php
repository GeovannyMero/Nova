				...
				<div class="tocify-content">
					<div class="icon-collection">
						<div class="fontawesome-icon-list">
							<a href="{{url('/ingresarInsumo')}}"><div class="fa-hover"><div class="icon-str"><i class="fa fa-plus"></i> <span>Agregar Insumos</span></div></div></a>
						</div>
					</div>
				</div>
			</div>
		...
		<table id="example-1" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
			...
			@foreach($producto as $i)
					...
					<tr>
						...
						<td>
							<a href="{{ url('insumo/edit',$i->eCodReg) }}"><button class="btn btn-secondary"><i class="fa fa-pencil"></i></button></a>
...
<script>
	...
	//alert(id);
bootbox.confirm("Est&aacute; seguro que desea eliminar el registro del insumo con c&oacute;digo No. "+id+"?",function(result){
	    if(result){
	    route = "{{url('insumo/delete' )}}" + "/" + id;
	    	$.ajax({
	    		type: 'GET',
	    		url: route,
	    		 success: function() {
